...
import { generateTreeForBundler, getLasComponentFromPath } from '../utils';
...
const EditorContainer = ({ width }: iProps) => {
  ...
  useEffect(() => {
    if (previousFiles !== undefined) {
      ...
      for (const file of files) {
        if (previousFiles.find((f) => f.path === file.path) === undefined) {
  ...
  const _onBundle = () => {
    dispatch(
      bundler({
        entryPoint: getLasComponentFromPath('src/index.tsx'),
