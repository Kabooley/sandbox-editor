...
const ScrollableElement: React.FC<iProps> = ({
  ...
  const [hoveringOnContainer, setHoveringOnContainer] = useState<boolean>(false)
  ...
  const _onScroll = (
    ...
    if (!disableHorizontalScrollbar) {
      ...
      const maximumScrollLeft = refScrollableElement.current.scrollWidth - width
      if (_scrollLeft < 0) _scrollLeft = 0
      if (_scrollLeft > maximumScrollLeft) _scrollLeft = maximumScrollLeft
      // // DEBUG:
  ...
  const getTopOfVerticalScrollbarThumb = () => {
    ...
    const maximumVerticalThumbTop = height * (1 - _rateHeightInScrollHeight)
    const rateOfScrollTopInScrollRange = scrollTop / (scrollHeight - height)
    let verticalThumbTop = rateOfScrollTopInScrollRange * (height - thumbHeight)
  ...
  const getLeftOfHorizontalScrollbarThumb = () => {
    ...
    const maximumHorizontalThumbLeft = width * (1 - _rateWidthInScrollWidth)
    const rateOfScrollLeftInScrollRange = scrollLeft / (scrollWidth - width)
