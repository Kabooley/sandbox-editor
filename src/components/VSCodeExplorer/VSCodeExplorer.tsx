...
const VSCodeExplorer: React.FC<iProps> = ({ width, height }) => {
   ...
   **/
  const [stackOneHeight, setStackOneHeight] = useState<number>(_minConstraints);
  ...
  const getMaxConstraints = (collapsings?: number) => {
    ...
    const _collapsings =
      collapsings === undefined
        ? [collapseStackOne, collapseStackTwo, collapseStackThree].filter(
            Boolean
          ).length
        : collapsings;
    ...
    switch (_collapsings) {
      ...
      default: {
         ...
         * すべてのstackが開いているとき
         * */
        return explorerHeight - explorerHeight * rateOfMinHeightOfSection * 2;
      }
    }
  };
