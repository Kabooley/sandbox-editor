...
const Workspace: React.FC<iProps> = ({
  ...
  const handleDeleteNode = (_explorer: iExplorer) => {
    ...
    if (deletionTargetFiles.length > 1) {
      const deletionTargetFilesPath = deletionTargetFiles.map((df) => df.path)
      dispatch(
  ...
  const handleReorderNode = (droppedId: string, draggableId: string): void => {
    ...
    const movingItem: iExplorer | undefined = getNodeById(treeData, draggableId)
              ...
              return {
                ...
                changeProp: {
                  newPath:
                    ...
                    r.path
                      .split('/')
                      .slice(movingFilePathArr.length - 1, r.path.length)
                      .join('/'),
                },
              }
            }),
