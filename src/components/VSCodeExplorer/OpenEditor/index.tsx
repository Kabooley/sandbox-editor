...
const OpenEditor: React.FC<iProps> = ({
   ...
   *
   ************************************************/
  const handleClickFile = (e: React.MouseEvent<HTMLDivElement>, file: File) => {
    e.stopPropagation();
  ...
  const renderActionCloseAllEditors = () => {
    ...
    return <Action handler={clickHandler} icon={closeAllIcon} altMessage="" />;
  };
  ...
  const renderActionSaveAllFiles = () => {
    ...
    return <Action handler={clickHandler} icon={saveAllIcon} altMessage="" />;
  };
        ...
        >
          ...
          <div className="actions always-appear">
            <div className="actions-bar">
              <ul className="actions-container">{renderActionCloseAFile(f)}</ul>
            </div>
          </div>
          <div className="codicon">
            <img src={chevronRightIcon} />
          </div>
          <h3 className="item-label">{getFilenameFromPath(f.getPath())}</h3>
          <span>{f.getPath()}</span>
        </div>
      ))}
    </Stack>
