 ...
 *
 * DEBUG:
 * - DEBUG: ../../TestTypingLibsでこのファイルの挙動確認をするため、一時的に使わないmonacoをコメントアウトしている
 * *************************************************************************/
...
console.log('[typingLibsSlice] generated worker');
...
const addExtraLibs = (code: string, path: string) => {
    // DEBUG:
    console.log(`[typingLibsSlice] addExtraLibs: ${path}`);
...
export const fetchModule = createAsyncThunk(
    ...
    async ({ moduleName, version }: ModuleAndVersion) => {
        return api
...
/***
 * @param {Array<ModuleAndVersion>} deletionModules - Modules to be requested to be deleted.
 * @returns {Array<string>} - Deleted module's dependencies path.
 *
 * Remove requested modules from dependencies
 * by removing from cache and monaco extraLibs.
 * */
export const removeModule = createAsyncThunk(
    'typingLibs/removeModule',
    async (deletionModules: ModuleAndVersion[]) => {
        let deletedDependencies: string[] = [];
        return Promise.all([
            ...deletionModules.map((dm) =>
                api
                    .isAlreadyExist(dm.moduleName, dm.version)
                    .then((isExist: boolean) => {
                        if (isExist) {
                            return api
                                .removeLibs(dm.moduleName, dm.version)
                                .then((deleted) => {
                                    deletedDependencies = [
                                        ...deletedDependencies,
                                        ...deleted,
...
const typingLibsSlice = createSlice({
    ...
    extraReducers(builder) {
        builder
            // state.dependenciesから該当のモジュールのstateを`loading`にする
            // TODO: この時点でリクエストされたモジュールをdependenciesに加えるべきか？
            .addCase(fetchModule.pending, (state, action) => {
                // DEBUG:
                console.log(
                    `[typingLibsSlice] start fetching ${action.meta.arg.moduleName}@${action.meta.arg.version}`
                );

                const { moduleName, version } = action.meta.arg;
            ...
            .addCase(fetchModule.fulfilled, (state, action) => {
                // DEBUG:
                console.log(
                    `[typingLibsSlice] fulfilled ${action.meta.arg.moduleName}@${action.meta.arg.version}`
                );
            ...
            .addCase(fetchModule.rejected, (state, action) => {
                ...
                // DEBUG:
                console.log(
                    `[typingLibsSlice] rejected ${action.meta.arg.moduleName}@${action.meta.arg.version}`
                );
        ...
        builder
            ...
            .addCase(removeModule.fulfilled, (state, action) => {
                ...
                action.payload.forEach((deleted) => {
                    const cachedLib = typingLibs.get(deleted);
                    if (cachedLib) {
                        cachedLib.js.dispose();
                        cachedLib.ts.dispose();
                    }
...
// type iTypingLibsActions =
//   ActionMap<iTypingLibsActionPayload>[keyof ActionMap<iTypingLibsActionPayload>];

/***
 * stores of dependencies of dependency.
 *
 * Library paths related to requested module and
 * requested module name pair will be registered.
 *
 * TODO: fetchLibs.worker.tsのほうに同じキャッシュ機能があるので、こちらは不要かどうか検討すること
 * */
// const setOfDependencies = new Map<string, string[]>();
