...
function filesReducer(files: File[], action: iFilesActions) {
  switch (action.type) {
    ...
    case "DELETE_MULTIPLE_FILES": {
      ...
      console.log(`[FilesContext] DELETE_MULTIPLE_FILES: ${requiredPaths}`);

      const updatedFiles: File[] = files.filter((f) => {
        return requiredPaths.find((r) => r === f.getPath()) === undefined
          ? true
          : false;
      });
    ...
    case "CHANGE_FILE": {
      ...
      const updatedFiles = files.map((f) => {
        if (f.getPath() === targetFilePath) {
          ...
          changeProp.newPath !== undefined && clone.setPath(changeProp.newPath);
    ...
    case "CLOSE_FILE": {
      ...
      console.log(`[FilesContext] OPEN_FILE: ${path}`);

      // Was target file `isSelected` true?
      let nextSelected: File | undefined;
      if (target.isSelected()) {
        nextSelected = files.find((f) => f.isOpening() && !f.isSelected());
      }
      ...
      const updatedFiles = files.map((f) => {
        ...
        else if (nextSelected && f.getPath() === nextSelected.getPath()) {
          const clone: File = Object.assign(
            Object.create(Object.getPrototypeOf(f)),
            f
          );
