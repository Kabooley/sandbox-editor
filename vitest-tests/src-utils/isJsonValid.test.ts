...
const cases = [
  { describe: 'Should be valid', str: jsonStr, shouldBe: true },
  { describe: 'Should be invalid', str: incompleteJSON, shouldBe: false },
];
...
describe('Test isJsonString()', () => {
  cases.forEach((c) => {
    it(c.describe, () => {
      expect(isJsonValid(c.str)).toBe(c.shouldBe);
    });
  });
});
